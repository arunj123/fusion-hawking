...
// Very minimal JSON config loader for specific fusion-hawking schema
// Limitations: No full JSON support, assumes strict structure as per config.json
...
public:
    static InstanceConfig Load(const std::string& path, const std::string& instance_name) {
        ...
        // Find closing brace of this instance
        ...
        if (prov_pos != std::string::npos) {
            ...
            std::string prov_block = block.substr(p_start, p_end - p_start);
            ParseProviding(prov_block, config.providing);
        }
...
private:
    static void ParseProviding(const std::string& json, std::map<std::string, ServiceConfig>& map) {
        ...
        while ((pos = json.find("\"", pos)) != std::string::npos) {
            ...
            if (key == "providing" || key == "service_id" || key == "instance_id") { pos = key_end + 1; continue; } // Skip nested keywords if any match? No, "providing" is parent.
