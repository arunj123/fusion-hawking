"""
Rust code generator for Fusion Hawking.

Generates per-service Rust binding files with typed server stubs and client proxies.
"""
...
class RustGenerator(AbstractGenerator):
    ...
    def generate(self, structs: list[Struct], services: list[Service], output_dir: str = "build/generated") -> dict[str, str]:
        ...
        # 1. Types file (shared @dataclass types)
        if structs:
            files[os.path.join(rust_dir, "types.rs")] = self._generate_types_file(structs)

        # 2. Per-service files
        for svc in services:
            snake = self._to_snake(svc.name)
            files[os.path.join(rust_dir, f"{snake}.rs")] = self._generate_service_file(svc, structs)

        # 3. mod.rs (links everything together)
        files[os.path.join(rust_dir, "mod.rs")] = self._generate_mod_rs(structs, services)

        return files

    def _generate_mod_rs(self, structs: list[Struct], services: list[Service]) -> str:
        lines = [
            "// Auto-generated by Fusion Hawking Codegen -- DO NOT EDIT",
            "",
        ]
        if structs:
            lines.append("pub mod types;")
            lines.append("#[allow(unused_imports)]")
            lines.append("pub use types::*;")
            lines.append("")
        for svc in services:
            snake = self._to_snake(svc.name)
            lines.append(f"pub mod {snake};")
            lines.append(f"#[allow(unused_imports)]")
            lines.append(f"pub use {snake}::*;")
        lines.append("")
        return "\n".join(lines)
    ...
    def _generate_service_file(self, svc: Service, all_structs: list[Struct]) -> str:
        ...
        if all_structs:
            lines.append("#[allow(unused_imports)]")
            lines.append("use super::types::*;")
        lines.append("")

        # Request/Response/Event structs
        ...
        # Provider Trait
        lines.append(self._generate_provider_trait(svc, pasc))

        # Server Stub
        lines.append(self._generate_server_stub(svc, pasc))

        # Client Proxy
        lines.append(self._generate_client_proxy(svc, pasc))
    ...
    def _generate_server_stub(self, svc: Service, svc_pascal: str) -> str:
        ...
        for f in svc.fields:
            if f.get_id:
                lines.append(f"    pub const FIELD_GET_{f.name.upper()}: u16 = {f.get_id};")
            if f.set_id:
                lines.append(f"    pub const FIELD_SET_{f.name.upper()}: u16 = {f.set_id};")
            if f.notifier_id:
                lines.append(f"    pub const EVENT_{f.name.upper()}_NOTIFY: u16 = {f.notifier_id};")
        lines.append("}")
    ...
    def _generate_client_proxy(self, svc: Service, svc_pascal: str) -> str:
        ...
        for m in svc.methods:
            ...
            lines.append(f"        req.serialize(&mut payload)?;")
